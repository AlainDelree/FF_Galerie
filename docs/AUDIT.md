...
- **A2 — `commitMulti` force:true** : ✅ résolu (`e06b088`, 27/06). Le fallback `force:true` (écrasement silencieux) remplacé par une boucle d'optimistic concurrency : blobs créés une fois, puis max 3 tentatives qui relisent HEAD + reconstruisent arbre/commit (nouveau `base_tree`/`parent`) + `PATCH force:false`, avec backoff exponentiel + jitter. On se rebase au lieu d'écraser. Alerte email+Issue (`rapporterErreur`) dès la 2e tentative, gâtée par la constante `ALERTER_RETENTATIVE` (`admin.js`) ; alerte d'échec des 3 tentatives toujours active (priorité bloquant) + `throw`. Journal des tentatives (HEAD attendu/réel, délais) joint comme preuve. **Validé en réel le 27/06** : un hiccup transitoire de GitHub (faux « not a fast forward » sur un ref fraîchement poussé, consistance éventuelle) a déclenché le retry ; relecture → même HEAD → succès, **0 perte** (le commit final était bien parenté sur le HEAD attendu, donc aucun éditeur concurrent réel). Apprentissage : ces hiccups transitoires sont plus fréquents que la vraie course multi-personnes — l'alerte peut sonner même en solo, surtout juste après un push.
